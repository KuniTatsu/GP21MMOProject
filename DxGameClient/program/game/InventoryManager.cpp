...
void InventoryManager::AddItemToInventory(const int ItemId, std::vector<std::shared_ptr<Inventory>>& Inventories, int& InventoryNum)
{
	...
	auto itemMaster = iManager->GetItemData(ItemId);
	//装備アイテムだったら
	if (iManager->GetItemData(1) >= 2) {
		...
		//文字列データの取得
		std::vector<std::string> stringData = iManager->GetAllStringData();
		
		//装備アイテムを生成 生成時にステータスをランダムに変更
		auto neweItem =  std::make_shared<EquipItem>();
		
		//インベントリ追加
		Inventories[InventoryNum]->AddInventory(neweItem);
	}
	else {
		std::vector<int> intData = iManager->GetAllIntData();
		std::vector<std::string> stringData = iManager->GetAllStringData();
