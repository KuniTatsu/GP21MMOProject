...
UIManager::UIManager()
{
	...
	LoadUI(TOOLTIPUIPASS, makedToolTipUI);
	LoadUI(CHATAREAUIPASS, makedChatAreaUI);
	LoadUI(WAIT, makedWaitUI);
}
...
void UIManager::LoadUI(std::string Pass, std::vector<std::shared_ptr<GraphicUI>>& putInVector)
{
	//CSV読み込み
	auto loadUICsv = tnl::LoadCsv(Pass);
	...
	for (int i = 1; i < loadUICsv.size(); ++i) {
		...
		if (type == static_cast<uint32_t>(LOADMODE::NORMAL)) {
			...
			newUI->ChangeLiveUI();
			//vectorに登録
			putInVector.emplace_back(newUI);
			//次のループへ
			continue;
		}
		if (loadUICsv[i][12] == "none") {
